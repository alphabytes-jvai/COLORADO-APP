...
export function SearchScreen({ visible, onClose }: SearchScreenProps) {
  ...
  const handleExploreItemPress = useCallback(
    (item: AllDataStructure) => {
      ...
      // Close the search modal first for iOS
      onClose();

      // Add slight delay for iOS modal transition
      setTimeout(
        () => {
          router.push({
  ...
  const handleResultPress = (result: AllDataStructure) => {
    ...
    // Close modal first for iOS
    onClose();

    // Navigate with delay for iOS modal transition
    setTimeout(
      () => {
        router.push({
          pathname: "/(main)/detail/[id]",
    ...
    >
      <SafeAreaView className="flex-1 bg-white">
        ...
        <View className="flex flex-row items-center justify-between px-4 py-3 bg-white border-b border-gray-100">
          ...
          <Text className="text-xl font-semibold text-gray-800 ml-3 flex-1">
        ...
        <ScrollView
          ...
          contentContainerStyle={{
            paddingBottom: Platform.OS === "ios" ? keyboardHeight : 20,
          }}
        >
          {searchQuery.trim() ? (
            // Search Results
